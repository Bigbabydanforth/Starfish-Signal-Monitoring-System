...
// We mock our own airtable wrapper (not the npm package) so routes get fake data.
...
// Dynamic import AFTER mock registration so app picks up the mocked module
...
describe('GET /api/signals', () => {
  ...
  it('ignores unknown query params safely', async () => {
    const res = await request.get('/api/signals?foo=bar&hack=true')
    assert.equal(res.status, 200)
  })

  it('accepts valid types filter without error', async () => {
    const res = await request.get('/api/signals?types=Job+Change')
    assert.equal(res.status, 200)
  })

  it('accepts valid priorities filter without error', async () => {
    const res = await request.get('/api/signals?priorities=HIGH')
    assert.equal(res.status, 200)
  })

  it('accepts combined type and priority filters', async () => {
    const res = await request.get('/api/signals?types=Rebrand&priorities=HIGH,LOW')
    assert.equal(res.status, 200)
  })
})
...
describe('GET /api/signals/:id', () => {
  ...
  it('returns 404 for an unknown id', async () => {
    const res = await request.get('/api/signals/recDOESNOTEXIST')
